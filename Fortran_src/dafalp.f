...
!>      types and for neutrons to incoming alpha spectra hitting Earth's atmosphere. 
...
!-----------------------------------------------------------
!> @param[in] spectrum.a a file containing the incoming spectrum of
!> alpha particles
!> @param[in] flightfile the file containing the coordinates where dose
!> rates will be calculated
!> @param[in] temp.a the path to the output file which will be generated
...
!> four files in /data/mcnpx/alpha/ relative to the \b $MAIREHOME environment
...
!> currently called from: $3$2$1.a $4 temp.a
!-----------------------------------------------------------
...
c     open and read in the response matrixs
      ...
      idx = index(path,' ')-1
      filnam = path(1:idx)//'/data/mcnpx/alpha/adose.rpf'
      open(21,file=filnam,form='formatted',status='old')
      filnam = path(1:idx)//'/data/mcnpx/alpha/edose.rpf'
      open(22,file=filnam,form='formatted',status='old')
      filnam = path(1:idx)//'/data/mcnpx/alpha/dosee.rpf'
      open(23,file=filnam,form='formatted',status='old')
      filnam = path(1:idx)//'/data/mcnpx/alpha/neutron.rpf'
...
c     
         do i = 1,50
            fl(i) = flux(i)*fr
         enddo
         ene = 1000.*(sqrt(rc**2/4.+ 0.938**2)- 0.938) ! per nucleon
         ie = 1
...
c
     ...
     +       rpf(layer,50+i)*f1*fl(i) + rpf(nlayer,50+i)*f2*fl(i)
            tn3 = tn3 +
     +       rpf(layer,100+i)*f1*fl(i) + rpf(nlayer,50+i)*f2*fl(i)
         enddo
         SEU = tn2 * 1e-13
         SEL = tn2 * 1e-8
c
